...
contract PlotsCore {
    ...
    mapping(address => address) public ActiveLoan; // Track active loan per user
    ...
    function BorrowToken(address Collection, uint256 TokenId, LengthOption Duration, OwnershipPercent Ownership) public payable {
        ...
        require(ActiveLoan[msg.sender] == address(0), "User already has an active loan"); // Check for active loan
        ...
        if (IERC721(Collection).ownerOf(TokenId) == Treasury && Ownership != OwnershipPercent.Zero) {
            ...
            TokenValue = 10000000000; //TODO: THIS IS BROKEN, FIGURE OUT A WORKAROUND
            ...
            Origin = LendContract;
            DurationUnix = (uint8(Duration) + 1) * 60; // Months system for peer-to-peer
        } 
        else {
        ...
        ActiveLoan[msg.sender] = LoanContract; // Track active loan
    }
    ...
    function CloseLoan(address LoanContract, bool relist) public{
        ...
        ActiveLoan[Borrower] = address(0); // Clear active loan
    }
